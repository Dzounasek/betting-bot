...
st.set_page_config(page_title="Value Bot v5.8 PRO", layout="wide")
...
# =========================================================
# LAYOUT: Rozdělení stránky na hlavní část (vlevo) a tiket (vpravo)
# =========================================================
main_col, ticket_col = st.columns([3, 1.2])

with ticket_col:
    # Fixní kontejner = vlastní posuvník na pravé straně!
    with st.container(height=850, border=True):
        ...
        if len(st.session_state.ticket) == 0:
            st.caption("Zatím žádné sázky. Klikni na ➕ u libovolného trhu vlevo.")
        else:
            total_stake = 0
            ...
            # Riziko: pravděpodobnostní shrnutí
...
with main_col:
    st.title("⚽ Betting Bot v5.8 PRO + Smart Tipovačka")
    st.markdown("Vylepšená AI, Smart skóre a oddělený tiket vpravo.")
    ...
    # --- 4. AI KOMENTÁŘ (Nové realističtější limity) ---
    ...
    if st.session_state.show_results:
        # Zvýšený důraz na xG (75 %) pro větší realističnost a menší šanci na 1:1
        ...
        if p_home > p_draw and p_home > p_away: 
            # Favoritem jsou domácí -> prohledáváme jen výsledky, kde domácí vyhrají
            ...
            # Favoritem jsou hosté -> prohledáváme jen výsledky pro hosty
            for i in range(max_g):
        ...
        st.caption("*(Poznámka: Výsledek do Tipovačky se nyní inteligentně odvíjí od celkového nejpravděpodobnějšího vítěze, aby maximalizoval zisk bodů v soutěžích.)*")
        st.divider()

        def add_to_ticket(label, prob, user_odds, stake_czk, ev):
            """Přidá sázku do tiketu v session state."""
